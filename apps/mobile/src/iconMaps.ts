...
/** Resolve avatar preferring flat ids, then legacy icon names, then emoji. */
export function resolveAvatar(value: string | null | undefined): FlatAvatarId {
  if (!value) return "ava-knight";
  if (value in FLAT_AVATAR_TONES) return value as FlatAvatarId;
  // Legacy icon name (pawn, cap, …) or already-normalized flat id miss above
  const fromLegacy = resolveFlatAvatar(value);
  if (fromLegacy !== "ava-knight" || value === "knight" || value === "ava-knight") return fromLegacy;
  return resolveFlatAvatar(emojiToIcon(value));
}

export { resolveFlatAvatar };
