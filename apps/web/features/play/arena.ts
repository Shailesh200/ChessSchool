...
/** Persisted summary when a 4-game arena run finishes. */
...
/** Four bots around the chosen ELO band — distinct names from tier profiles. */
...
/** Points table from games played (player sums all; each bot one game vs you). */
...
export function playerArenaPoints(run: ArenaRun): number {
  return run.results.reduce((sum, r) => sum + r.playerScore, 0);
}
...
/** XP bonus when a 4-game arena run finishes. */
export function arenaXpBonus(run: ArenaRun): number {
  const pts = playerArenaPoints(run);
  const base = 30;
  const perPoint = 12;
