...
const SPECS = {
  ...
  lesson: {
    web: async (p) => {
      await p.goto(`${PROD}/lesson/board-basics`, { waitUntil: "domcontentloaded" });
    },
    app: async (p) => {
      await p
  ...
  play: {
    web: async (p) => {
      await p.goto(`${PROD}/play`, { waitUntil: "domcontentloaded" });
    },
    app: async (p) => {
      await p
        .getByText("Play", { exact: true })
        .click()
        .catch(() => {});
    },
    ready: "Start match",
  ...
  profile: {
    web: async (p) => {
      await p.goto(`${PROD}/profile`, { waitUntil: "domcontentloaded" });
    },
    app: async (p) => {
      await p
        .getByText("Profile", { exact: true })
  ...
  settings: {
    web: async (p) => {
      await p.goto(`${PROD}/settings`, { waitUntil: "domcontentloaded" });
    },
  ...
  journal: {
    web: async (p) => {
      await p.goto(`${PROD}/journal`, { waitUntil: "domcontentloaded" });
    },
  ...
  dashboard: {
    web: async (p) => {
      await p.goto(`${PROD}/dashboard`, { waitUntil: "domcontentloaded" });
    },
  ...
  homework: {
    web: async (p) => {
      await p.goto(`${PROD}/plan`, { waitUntil: "domcontentloaded" });
    },
    app: async (p) => {
      await p
        .getByTestId("homework")
        .click()
        .catch(() => {});
      await p.waitForTimeout(1800);
    },
  ...
  review: {
    web: async (p) => {
      await p.goto(`${PROD}/review`, { waitUntil: "domcontentloaded" });
    },
    app: async (p) => {
      await p
  ...
  classes: {
    web: async (p) => {
      await p.goto(`${PROD}/library`, { waitUntil: "domcontentloaded" });
    },
  ...
  class: {
    web: async (p) => {
      await p.goto(`${PROD}/class/class-pieces`, { waitUntil: "domcontentloaded" });
    },
    app: async (p) => {
      await p
        .getByText("Profile", { exact: true })
      ...
      await p.waitForTimeout(1400);
      await p
        .getByTestId("hub-Library")
        .click()
        .catch(() => {});
      await p.waitForTimeout(1800);
      await p
        .getByTestId("class-class-pieces")
        .click()
        .catch(() => {});
      await p.waitForTimeout(1800);
    },
...
async function capture(kind) {
  ...
  const b = await chromium.launch({
    channel: "chrome",
    args: isApp ? ["--disable-web-security"] : [],
  });
  ...
  if (isApp) {
    ...
    if (/onboarding|welcome/.test(p.url()))
...
const wW = (await sharp(wB).metadata()).width,
  aW = (await sharp(aB).metadata()).width,
  gap = 30;
await sharp({
  create: { width: wW + aW + gap, height: ht, channels: 4, background: "#cbd5e1" },
})
  .composite([
    { input: wB, left: 0, top: 0 },
    { input: aB, left: wW + gap, top: 0 },
  ])
  .png()
